...
console.log('TOWLB:', JSON.stringify(E(`(function(){BP.CUR='UK';for(var k in BP.BUY_CACHE)delete BP.BUY_CACHE[k];var q=BP.getBuyQtys('TOWLB-CAB-LG-LTBLU-R','UK');return {b3u:q.b3u,sea:q.b3uSea,air:q.b3uAir,b3:q.b3};})()`)));
...
console.log('totals:', JSON.stringify(E(`(function(){var MK=['UK','US','EU','AU','CA'];var t={b3u:0,sea:0,air:0,n:0};MK.forEach(function(m){BP.CUR=m;for(var k in BP.BUY_CACHE)delete BP.BUY_CACHE[k];var vis=BP.getFilteredVis()||[];vis.forEach(function(s){var q=BP.getBuyQtys(s.s,m);t.b3u+=q.b3u||0;t.sea+=q.b3uSea||0;t.air+=q.b3uAir||0;if((q.b3uSea||0)+(q.b3uAir||0)>0)t.n++;});});return t;})()`)));
console.log('headers:', E(`(function(){ if(!document.querySelector('.wrap')){var w2=document.createElement('div');w2.className='wrap';document.body.appendChild(w2);} if(!document.getElementById('main-hdr')){var t=document.createElement('table');var th=document.createElement('thead');th.id='main-hdr';t.appendChild(th);document.querySelector('.wrap').appendChild(t);} renderBuyView('buy'); var h=document.getElementById('main-hdr').innerHTML; return 'UrgSea='+(h.indexOf('Urgent<br>Sea')>=0)+' UrgAir='+(h.indexOf('Urgent<br>Air')>=0); })()`));
