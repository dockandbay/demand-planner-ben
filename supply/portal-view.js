      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
          ...
          function ppProdSamples(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            ...
            Promise.all([getJSON(EP.productSamplesBase+encodeURIComponent(ref)), fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};})]).then(function(_res){ var list=Array.isArray(_res[0])?_res[0]:[];
              ...
              function shipMenu(q){ q=(q||'').toLowerCase().trim();
                var opts=SRS.filter(function(sr){ return !q || (sr.ref+' '+sr.carrier+' '+sr.tracking).toLowerCase().indexOf(q)>=0; });
                ...
                html+='<div class="pp-ship-opt" data-val="not_shipped" style="padding:6px 9px;cursor:pointer;font-size:12px;border-bottom:1px solid #f1f5f9;color:#92400e">Not shipped</div>';
                html+='<div class="pp-ship-opt" data-val="" style="padding:6px 9px;cursor:pointer;font-size:12px;color:#64748b">— Unassign —</div>';
                return html; }
              ...
              var rows=list.slice().reverse().map(function(s){ var ph=(s.photos||[]).map(function(p){ var url=(EP.attachImgBase||'/api/supply/portal-attachment/')+p.id, canDel=p.uploader_kind==='supplier';
                ...
                return '<div style="border:1px solid #cbd5e1;border-radius:8px;padding:11px 13px;margin-bottom:14px;text-align:left"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><b style="font-family:ui-monospace,Menlo,monospace">'+esc(s.ref)+'</b>'+stBadge(s)+'<span class="mut tiny">'+esc(s.sample_date||'')+'</span>'
                  ...
                  +'<div style="margin-top:6px;display:flex;gap:8px;align-items:center;flex-wrap:wrap"><span class="mut tiny">Status</span><select class="pp-samp-status fci" data-id="'+s.id+'" style="font-size:11px">'+SST.map(function(o){return '<option value="'+o[0]+'"'+(s.supplier_status===o[0]?' selected':'')+'>'+o[1]+'</option>';}).join('')+'</select><span class="mut tiny" style="margin-left:4px">Shipment</span><span class="pp-ship-wrap" data-id="'+s.id+'" style="position:relative;display:inline-block"><input class="pp-ship-inp fci" data-id="'+s.id+'" value="'+esc(curShipLabel(s))+'" placeholder="search shipment / tracking…" autocomplete="off" style="width:240px;font-size:11px;text-align:left"><div class="pp-ship-menu" data-id="'+s.id+'" style="display:none;position:absolute;top:100%;left:0;z-index:60;background:#fff;border:1px solid #cbd5e1;border-radius:6px;max-height:220px;overflow:auto;width:250px;box-shadow:0 8px 22px rgba(0,0,0,.16)"></div></span><a class="pp-ship-new" data-id="'+s.id+'" style="font-size:11px;color:#16a34a;cursor:pointer;margin-left:8px;white-space:nowrap">＋ new shipment</a></div>'
                  +(needsShip(s)?'<div style="margin-top:5px;font-size:11px;color:#b45309;background:#fffbeb;border:1px solid #fde68a;border-radius:6px;padding:4px 8px">⚠ Completed but not on a sample shipment — assign it to a shipment above, or choose <b>Not shipped</b>.</div>':'')
              ...
              box.querySelectorAll('.pp-ship-inp').forEach(function(inp){ var menu=box.querySelector('.pp-ship-menu[data-id="'+inp.dataset.id+'"]'); if(!menu)return;
                function openMenu(){ menu.innerHTML=shipMenu(inp.value); menu.style.display=''; }
                inp.onfocus=function(){ inp.dataset.prev=inp.value; inp.value=''; openMenu(); };
                inp.oninput=openMenu;
                inp.onblur=function(){ setTimeout(function(){ menu.style.display='none'; if(inp.value===''&&inp.dataset.prev)inp.value=inp.dataset.prev; },200); };
                menu.onmousedown=function(e){ var opt=e.target.closest&&e.target.closest('.pp-ship-opt'); if(!opt)return; e.preventDefault(); menu.style.display='none'; assignShip(inp.dataset.id, opt.getAttribute('data-val')); }; });
              box.querySelectorAll('.pp-ship-new').forEach(function(a){ a.onclick=function(){ if(!confirm('Create a new sample shipment with this sample on it? You can add the address & tracking in the Samples tab.'))return; postJSON(EP.sampleCreate,{dev_samples:[{id:a.dataset.id,qty:1}]},function(j){ if(j&&j.error){alert(j.error);return;} if(j&&j.ref)ppSetHash('samples',j.ref); reload(); }); }; });
