...
scope.querySelectorAll('.pp-dl-custom').forEach(function(btn){ btn.onclick=function(){ if(BC.placeholder){BC.note();return;} btn.disabled=true; fetch(EP.labelData+'?po='+encodeURIComponent(btn.dataset.po)+'&project='+encodeURIComponent(btn.dataset.pid)).then(function(r){return r.json();}).then(function(rows){ btn.disabled=false; if(rows&&rows.error){ppNotice(rows.error);return;} if(!rows||!rows.length){ppNotice('None of this PO\'s SKUs have a custom barcode in this project');return;} BC.sheets(rows,['product','carton','inner'],btn.dataset.po+'_'+String(btn.dataset.name||'custom').replace(/[^\w\-]+/g,'_')+'_custom_barcodes.zip',btn); }).catch(function(){ppNotice('Could not load custom barcodes');btn.disabled=false;}); }; });   // v27.570: project-linked custom numbers only
