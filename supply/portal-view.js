          ...
          function ppProdSamples(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            ...
            Promise.all([getJSON(EP.productSamplesBase+encodeURIComponent(ref)), fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};})]).then(function(_res){ var list=Array.isArray(_res[0])?_res[0]:[];
              ...
              box.innerHTML='<div style="text-align:left"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="font-weight:700;font-size:12.5px">Sample versions</div><button class="save-btn pp-add-sample" style="background:var(--pos);color:#fff;border-color:var(--pos);font-size:12px">+ Add sample</button></div>'
                ...
                +'<div style="margin-bottom:8px"><label style="font-size:12px">Sample shipment <select class="fci ps2-ship" style="font-size:12px;margin-left:4px"><option value="">— assign later —</option>'+SRS.map(function(sr){return '<option value="'+sr.id+'">'+esc(sr.ref)+'</option>';}).join('')+'<option value="__new__">+ New shipment (create now)</option><option value="not_shipped">Not shipped</option></select></label> <span class="mut tiny">optional now — can set later per sample</span></div>'
              ...
              var sv=box.querySelector('.ps2-save'); sv.onclick=function(){ var msg=box.querySelector('.ps2-msg');
                ...
                var shipv=(box.querySelector('.ps2-ship')||{}).value||'', mkNew=(shipv==='__new__'), shipFields=shipv==='not_shipped'?{not_shipped:true}:((shipv&&!mkNew)?{sample_request_id:shipv}:{});   // v27.529: __new__ = create a shipment for this sample right after it is created
