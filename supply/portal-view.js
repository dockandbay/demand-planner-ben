      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
          ...
          function ppProdMaster(box, ref, dispRef){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).then(function(d){ if(!d||!d.item){ box.innerHTML='<div class="mut" style="text-align:left">Not found.</div>'; return; } var it=d.item;   // v27.848 (Ben): real portal → the supplier-scoped /api/portal/product-item; admin preview falls back to the admin endpoint (still filtered client-side)
              ...
              var _COMPS=(d.components!=null) ? (d.components||[]).map(function(c){ return [c.dimension||('comp:'+c.id), c.name]; })
                : [['product','Product'],['packaging','Packaging'],['labels','Labels/wraps'],['polybag','Polybags'],['other','Other components']];
              ...
              var sizes=(d.sizes||[]).map(function(s){ function sd(dm){ return ((s.dimensions||[]).filter(function(x){return x.dimension===dm;})[0])||{}; } var pk=sd('packaging'), ok=s.approval_status==='approved';
                ...
                var body='<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="color:var(--faint);font-size:10.5px;text-transform:uppercase;letter-spacing:.04em"><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Component</th><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Status</th><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Sample versions</th><th style="text-align:left;padding:0 0 4px;font-weight:600">Files</th></tr></thead><tbody>'
                  +_COMPS.map(function(cc){ var _dim=cc[0], _sd=sd(_dim), _lbl=cc[1]; if(_dim==='packaging'&&_sd.packaging_type)_lbl+=' ('+esc(_sd.packaging_type)+')'; return pDimRow(_lbl, _sd, s, _dim); }).join('')+'</tbody></table>';   /* v27.845 (Ben): components now match the admin /sizes view exactly (driven by the product's real components, not a hardcoded 5) */
