      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
          ...
          function ppProdSamples(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            ...
            Promise.all([getJSON(EP.productSamplesBase+encodeURIComponent(ref)), fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};})]).then(function(_res){ var list=Array.isArray(_res[0])?_res[0]:[];
              ...
              var SRS=(((typeof _ppData!=='undefined'&&_ppData&&_ppData.samples)||[])).map(function(sr){return {id:sr.id,ref:sr.ref};});
              ...
              function curShip(s){ if((s.shipments||[]).length)return String(s.shipments[0].id); if(s.not_shipped)return 'not_shipped'; return ''; }
              function shipSel(cur){ return '<option value=""'+(cur===''?' selected':'')+'>— assign shipment —</option>'+SRS.map(function(sr){return '<option value="'+sr.id+'"'+(cur===String(sr.id)?' selected':'')+'>'+esc(sr.ref)+'</option>';}).join('')+'<option value="not_shipped"'+(cur==='not_shipped'?' selected':'')+'>Not shipped</option>'; }
              ...
              box.querySelectorAll('.pp-samp-status').forEach(function(sel){ sel.onchange=function(){ postJSON(EP.productSample+'/'+sel.dataset.id+'/status',{supplier_status:sel.value},function(j){ if(j&&j.error){alert(j.error);return;} ppProdSamples(box,ref); }); }; });
              box.querySelectorAll('.pp-samp-ship').forEach(function(sel){ sel.onchange=function(){ var v=sel.value, body=v==='not_shipped'?{mode:'not_shipped'}:(v?{mode:'shipment',sample_request_id:v}:{mode:'unassigned'}); postJSON(EP.productSample+'/'+sel.dataset.id+'/assign',body,function(j){ if(j&&j.error){alert(j.error);return;} ppProdSamples(box,ref); }); }; });
