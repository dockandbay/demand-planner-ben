    ...
    function downloadProductionPlan(){
      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
          ...
          function renderPP(){ if(!_ppData)return; var body=document.getElementById('pp-body');
            ...
            setSampBadge(); setPosBadge(); setShipBadge(); setProdBadge(); setSpecBadge(); try{ renderPortalNotif(); }catch(e){} ppSyncSec(); if(PP_LANG==='zh')try{ ppTranslate(document.body); }catch(e){}   // re-sync after the badges are set (section counts, v27.506)
