      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
          ...
          function ppProdSamples(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            ...
            Promise.all([getJSON(EP.productSamplesBase+encodeURIComponent(ref)), fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};})]).then(function(_res){ var list=Array.isArray(_res[0])?_res[0]:[];
              ...
              function newShipmentFor(id){ if(!confirm('Create a new sample shipment with this sample on it? You can add the address & tracking in the Samples tab.'))return; postJSON(EP.sampleCreate,{dev_samples:[{id:id,qty:1}]},function(j){ if(j&&j.error){alert(j.error);return;} if(j&&j.ref)ppSetHash('samples',j.ref); reload(); }); }
              function paint(){
              ...
              var rows=list.slice().reverse().map(function(s){ var ph=(s.photos||[]).map(function(p){ var url=(EP.attachImgBase||'/api/supply/portal-attachment/')+p.id, canDel=p.uploader_kind==='supplier';
                ...
                return '<div style="border:1px solid #cbd5e1;border-radius:8px;padding:11px 13px;margin-bottom:14px;text-align:left"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><b style="font-family:ui-monospace,Menlo,monospace">'+esc(s.ref)+'</b>'+stBadge(s)+'<span class="mut tiny">'+esc(s.sample_date||'')+'</span>'
                  ...
                  +'<div style="margin-top:6px;display:flex;gap:8px;align-items:center;flex-wrap:wrap"><span class="mut tiny">Status</span><select class="pp-samp-status fci" data-id="'+s.id+'" style="font-size:11px">'+SST.map(function(o){return '<option value="'+o[0]+'"'+(s.supplier_status===o[0]?' selected':'')+'>'+o[1]+'</option>';}).join('')+'</select><span class="mut tiny" style="margin-left:4px">Link to shipment</span><span class="pp-ship-wrap" data-id="'+s.id+'" style="position:relative;display:inline-block"><input class="pp-ship-inp fci" data-id="'+s.id+'" value="" placeholder="search shipment / tracking, or add new…" autocomplete="off" style="width:260px;font-size:11px;text-align:left"><div class="pp-ship-menu" data-id="'+s.id+'" style="display:none;position:absolute;top:100%;left:0;z-index:60;background:#fff;border:1px solid #cbd5e1;border-radius:6px;max-height:240px;overflow:auto;width:270px;box-shadow:0 8px 22px rgba(0,0,0,.16)"></div></span></div>'
                  ...
                  +((s.shipments||[]).length?'<div style="margin-top:7px;display:flex;flex-direction:column;gap:5px"><div class="mut tiny">Linked shipment'+((s.shipments||[]).length>1?'s':'')+'</div>'+(s.shipments||[]).map(function(sh){return '<div style="padding:6px 9px;background:#f0f6ff;border:1px solid #dbeafe;border-radius:6px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">📦 <b>'+esc(sh.ref)+'</b>'+(sh.carrier?'<span class="mut tiny">'+esc(sh.carrier)+'</span>':'')+(sh.tracking?'<span style="font-size:12px">'+carrierTrackLink(sh.carrier,sh.tracking)+'</span>':'')+'<a class="pp-ship-unlink" data-id="'+s.id+'" data-sr="'+sh.id+'" title="unlink from this shipment" style="margin-left:auto;color:#dc2626;cursor:pointer;font-size:12px;white-space:nowrap">✕ unlink</a></div>';}).join('')+'</div>':'')
