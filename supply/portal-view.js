            ...
            fetch(EP.shipmentNotesBase+encodeURIComponent(ref)).then(function(r){return r.json();}).then(function(notes){ shortNotes(notes);
              ...
              box.innerHTML='<div style="font-weight:600;font-size:12px;margin-bottom:4px">Add timeline note</div>'
                ...
                +((notes&&notes.length)?notes.map(function(n){ var escBtn=(EP.escalate&&n.id===recentSupId)?'<button class="save-btn light sp-esc-note" data-ref="'+esc(ref)+'" data-msg="'+esc(n.body)+'" title="Escalate this message to Dock & Bay by email" style="flex:0 0 auto;color:#b91c1c;border-color:#fca5a5;white-space:nowrap">⚑ Escalate</button>':'';
                  return '<div class="tiny" style="margin:2px 0;display:flex;gap:8px;align-items:flex-start">'+(escBtn?'<div style="flex:0 0 auto;min-width:78px">'+escBtn+'</div>':'')+'<div style="flex:1"><span class="mut">'+esc(n.created_at)+' · '+(n.author_kind==='supplier'?'You':'Dock &amp; Bay')+'</span> — '+esc(n.body)+'</div></div>';}).join(''):'<div class="mut tiny">No timeline entries yet.</div>');
