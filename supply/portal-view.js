...
(function(){
  ...
  function ppTranslate(root){ root=root||document.body; if(!root)return;
    ...
    root.querySelectorAll('[placeholder],[title]').forEach(function(el){ ['placeholder','title'].forEach(function(a){ if(!el.hasAttribute(a))return; var v=el.getAttribute(a), k=_i18nKey(v);
      if(PP_LANG==='zh'){ var zh=PP_ZH[k]; if(zh&&v!==zh){ if(!el.dataset['i18n'+a])el.dataset['i18n'+a]=v; el.setAttribute(a,zh); } }
  ...
  function hzTrackPillHtml(t){ if(!t||!t.status_code)return ''; var code=String(t.status_code).toLowerCase();
    var M={'delivered':['#e6f7ec','#1a7f37','Delivered'],'transit':['#e7effc','#1d4ed8','In transit'],'pre-transit':['#fff6e6','#b8860b','Pre-transit'],'failure':['#fdeaea','#c02626','Exception'],'unknown':['#f0f0f2','#6b7280','Unknown']};
    var m=M[code]||M['unknown']; var mark=code==='failure'?'▲':'●';
    var when=code==='delivered'?(t.delivered_at?' '+hzTrkDate(t.delivered_at):''):(t.eta?' · ETA '+hzTrkDate(t.eta):'');
  ...
  if(window.__PV_STANDALONE){ document.addEventListener('click',function(e){ var el=e.target&&e.target.closest?e.target.closest('.hz-trkpill[data-num]'):null; if(el){ e.preventDefault(); e.stopPropagation(); hzTrackLogOpen(el.getAttribute('data-num')); } }, true); }   // v27.855 (Ben): only in the real portal — in the admin app (which also loads portal-view.js) the admin's own pill handler + /api/tracking/detail is used
  ...
  function buildCartonSVG(kind,r,opts){ var W=600,H=841,cx=W/2,el=[];   // H matches the mould cell aspect (1178:1652)
    ...
    var _bl=(batch?'BATCH '+batch.batch:'')+(r.supplier_code?(batch?'   ':'')+r.supplier_code:'');   // batch number + 2-letter supplier code
    if(_bl) el.push(svgText(tx,bY+66,_bl,{size:15,bold:true}));
    y=bY+swW+34;
  ...
  function dlShipsWith(po,btn,base,air){ var orig=btn?btn.textContent:''; if(btn)btn.disabled=true;
    fetch((base||'/api/supply/ships-with/')+encodeURIComponent(po)).then(function(r){return r.json();}).then(function(d){
      if(!d||d.error){ if(btn){btn.textContent=orig;btn.disabled=false;} ppNotice((d&&d.error)||'Could not load ships-with data'); return; }
      Promise.all([fetchImgDataUri('/api/portal/asset/gotham-book'),fetchImgDataUri('/api/portal/asset/gotham-bold')]).then(function(a){
        ...
        preloadFonts(opts).then(function(){ rasterizeSVGCanvas(buildShipsWithSVG(d,opts),2.5,function(cv){ if(btn){btn.textContent=orig;btn.disabled=false;} if(!cv){ppNotice('Could not render label');return;}
          cv.toBlob(function(jb){ if(!jb){ppNotice('Could not encode label');return;} jb.arrayBuffer().then(function(ab){ bcDownloadBlob((air?'AIRFREIGHT-':'SHIPSWITH-')+zipSafe(po)+'_A4.pdf', new Blob([pdfA4(new Uint8Array(ab),cv.width,cv.height,MOLD_4)],{type:'application/pdf'})); }); },'image/jpeg',0.92); }); });
      });
  ...
  function bcMakeLabel(kind,r,batch,rrp){ if(!r||!confirmNoBatch(batch))return; bcGatherAssets(kind,r,batch,rrp,function(opts){
    rasterizeSVG(buildLabelSVG(kind,r,opts),2,function(blob){ if(!blob){ ppNotice('Could not render label PNG'); return; } bcDownloadBlob(kindPrefix(kind)+r.sku+'_label.png',blob); }); }); }
  ...
  function bcMakeSheet(kind,r,batch,rrp){ if(!r||!confirmNoBatch(batch))return; var cells=kind==='product'?MOLD_36:MOLD_4;
    bcGatherAssets(kind,r,batch,rrp,function(opts){ rasterizeSVGCanvas(buildLabelSVG(kind,r,opts),2.5,function(cv){ if(!cv){ ppNotice('Could not render label'); return; }
      cv.toBlob(function(jb){ if(!jb){ ppNotice('Could not encode label'); return; } jb.arrayBuffer().then(function(ab){
  ...
  function bcDownloadCrossdock(rows,po,dispatchOrder,client,address,btn,zipname){
    ...
    if(!items.length){ ppNotice('No crossdock SKUs with a barcode on this PO'); return; }
    ...
    Promise.all([fetchImgDataUri('/api/portal/asset/gotham-book'),fetchImgDataUri('/api/portal/asset/gotham-bold'),fetchImgDataUri('/api/portal/asset/db')]).then(function(a){
      ...
      preloadFonts(base).then(function(){ var pages=[], i=0;
        ...
        function finish(){ if(btn){btn.textContent=orig;btn.disabled=false;} if(!pages.length){ppNotice('Nothing rendered');return;}
  ...
  function bcDownloadSheets(rows,kinds,zipname,btn){
    ...
    if(!items.length){ ppNotice('No barcodes here'); return; }
      ...
      preloadFonts(base).then(function(){ var files=[], i=0;
        function next(){ if(i>=items.length) return finish();
          ...
          swP.then(function(sw){ var kp=kinds.filter(function(k){ return rowHasKind(r,k); }), ki=0;
            ...
            nk(); }); }
        function finish(){ if(btn){btn.textContent=orig;btn.disabled=false;} if(!files.length){ppNotice('Nothing rendered');return;} bcDownloadBlob(zipname,new Blob([zipStore(files)],{type:'application/zip'})); }
        next();
      });
    }); }
...
@media (max-width:640px){
  ...
  #supply-root table.pp-tbl thead th:first-child,
  #supply-root table.pp-tbl tbody tr:not([id]) td:first-child{width:36px!important;min-width:36px!important;max-width:36px!important;overflow:visible;padding-left:2px!important;padding-right:2px!important}
  #supply-root table.pp-tbl thead th:nth-child(2),
  #supply-root table.pp-tbl tbody tr:not([id]) td:nth-child(2){left:36px;white-space:normal;word-break:break-all;max-width:12ch;min-width:0}
  ...
  function mount(opts){
    ...
    var BC=opts.bc||(typeof bcDownloadSheets==='function'?{sheets:bcDownloadSheets,crossdock:bcDownloadCrossdock}:{placeholder:true,note:function(){ppNotice('Labels unavailable.');}});
    ...
    // Ownership-guarded server-side (/api/portal/scan/:code), so a code only ever resolves the caller's own sample.
    function ppScanCardOpen(code){ code=String(code||'').trim().toUpperCase();
      var old=document.getElementById('pp-scc-ov'); if(old)old.remove();
      var ov=document.createElement('div'); ov.id='pp-scc-ov';
      ov.style.cssText='position:fixed;inset:0;z-index:100050;background:#f3f4f6;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:env(safe-area-inset-top,0) 0 env(safe-area-inset-bottom,0)';
      ov.innerHTML='<div style="max-width:520px;margin:0 auto;min-height:100%;background:#fff;box-shadow:0 0 40px rgba(0,0,0,.08)">'
        +'<div style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-bottom:1px solid #e5e7eb;position:sticky;top:0;background:#fff;z-index:2">'
          +'<b style="font-size:14px">📇 <span class="pp-i18n">Sample card</span></b><span style="font-family:ui-monospace,Menlo,monospace;color:#6b7280;font-size:12px">'+esc(code)+'</span>'
          +'<button id="pp-scc-x" aria-label="Close" style="margin-left:auto;width:36px;height:36px;border-radius:50%;border:1px solid #e5e7eb;background:#fff;font-size:16px;cursor:pointer">✕</button></div>'
        +'<div id="pp-scc-body" style="padding:14px 14px 48px"><div style="padding:20px 0;color:#6b7280">Loading…</div></div></div>';
      document.body.appendChild(ov); document.body.style.overflow='hidden';
      function close(){ try{ ov.remove(); }catch(e){} document.body.style.overflow=''; _ppRouting=true; try{ location.hash='#/product/product'; }catch(e){} setTimeout(function(){ _ppRouting=false; },30); if(_ppData)renderPP(); }
      ov.querySelector('#pp-scc-x').onclick=close;
      var body=ov.querySelector('#pp-scc-body');
      fetch('/api/portal/scan/'+encodeURIComponent(code)).then(function(r){return r.json();}).then(function(d){
        if(!d||!d.ok||!d.sample){ body.innerHTML='<div style="padding:18px;border:1px solid #f59e0b;background:#fffbeb;border-radius:8px;font-size:13px">No sample of yours matches code <b>'+esc(code)+'</b>.</div>'; return; }
        var s=d.sample;
        ...
        body.innerHTML='<div style="font-size:18px;font-weight:800">'+esc(s.item_ref)+' <span style="font-weight:600;font-size:14px;color:#6b7280">v'+esc(s.version)+'</span></div>'
          +'<div style="margin:10px 0">'+swatch+'</div>'
          +(idRows.length?('<div style="display:grid;grid-template-columns:auto 1fr;gap:2px 12px;font-size:13px;margin-bottom:14px">'+idRows.map(function(r){return '<span style="color:#6b7280" class="pp-i18n-wrap">'+esc(r[0])+'</span><b>'+esc(r[1])+'</b>';}).join('')+'</div>'):'')
          +recv
          +(review?('<div style="font-weight:700;margin-top:16px"><span class="pp-i18n">Dock &amp; Bay review</span></div>'+review):'')
          +photos
          +'<div style="margin-top:20px"><button id="pp-scc-full" style="width:100%;background:#111827;color:#fff;border:none;border-radius:9px;padding:12px;font-size:14px;font-weight:700;cursor:pointer"><span class="pp-i18n">Open full record</span> ↗</button></div>';
        var fb=body.querySelector('#pp-scc-full'); if(fb)fb.onclick=function(){ close(); _ppRouting=true; try{ location.hash='#/product/product/'+encodeURIComponent(s.item_ref); }catch(e){} setTimeout(function(){ _ppRouting=false; ppApplyHash(); },40); };
    ...
    try{
      ...
      // v27.636: on the standalone portal the level-1 section menu moves UP into the dark brand bar (.pv-top),
      // sitting after the supplier name / before the spacer, white-on-black. The admin config preview has no
      // .pv-top → the menu stays inline above the tabs (styled by hz-theme.css) exactly as before.
      var _pvTop=document.querySelector('.pv-top');
      if(_pvTop){ var _sp=_pvTop.querySelector('.sp'); if(_sp)_pvTop.insertBefore(_secRow,_sp); else _pvTop.appendChild(_secRow); _bar.parentNode.insertBefore(tabsEl, _bar); }
      else { _bar.parentNode.insertBefore(_secRow, _bar); _bar.parentNode.insertBefore(tabsEl, _bar); }
  ...
  function hzTlAttach(ta,opts){ opts=opts||{}; if(!ta)return null; if(ta._hzAtt)return ta._hzAtt; var files=[], notice=opts.notice||ppNotice;
    var box=document.createElement('div'); box.className='hz-tl-drop'; box.style.cssText='margin:4px 0 6px;max-width:'+(opts.maxWidth||'560px')+';text-align:left';
    box.innerHTML='<label style="display:flex;align-items:center;justify-content:center;gap:6px;border:1.5px dashed var(--line);border-radius:8px;padding:6px 10px;font-size:11px;color:var(--muted);cursor:pointer;background:var(--hover)">📎 <span>Choose files or drag &amp; drop</span><span style="opacity:.75">· max 4MB per file</span><input type="file" multiple accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.csv,.ai,.zip" style="display:none"></label><div class="hz-tl-flist" style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px"></div><div class="hz-tl-fmsg mut tiny"></div>';
  ...
  function hzTlUploader(kind,ref,extra){ return function(f,fields,cb){ var body=Object.assign({kind:kind,ref:ref},fields,extra||{}); fetch((EP.timelineAttachment||'/api/portal/timeline-attachment'),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}).then(function(r){ return r.text().then(function(t){ try{ return t?JSON.parse(t):{}; }catch(_){ return {error:'Server error ('+r.status+')'}; } }); }).then(function(j){ cb(j&&j.id||null, j&&j.error||null); }).catch(function(e){ cb(null, e&&e.message||'upload failed'); }); }; }
    ...
    function ppExpand(p, lines, notes, subs, i, costs, supSkus, xd, add){ var po=esc(p.po); costs=costs||{}; supSkus=supSkus||[]; xd=xd||{}; add=add||[];
      ...
      var timeline=confirmBar+prodBlock
        ...
        +'<div style="margin:0 0 8px;display:flex;gap:5px"><textarea class="pp-note-body fci" spellcheck="true" autocorrect="on" autocapitalize="sentences" lang="en-GB" data-po="'+po+'" rows="1" placeholder="Reply to Dock &amp; Bay…" style="flex:1;min-height:26px;max-width:420px;text-align:left"></textarea><button class="save-btn pp-note-post" data-po="'+po+'">Post</button></div>'
    ...
    function prodBatchPOs(){ return prodPOsInBatch(PORTAL_PROD_BATCH); }
    // v27.757 (Ben): PO approval from the PRODUCTIONS tab. Same state as the PO card's "Confirm order" (supplier_confirmed_at) —
    // one approval, two doors. 'pending' = Dock & Bay asked for confirmation and the supplier hasn't given it (or a change cleared it).
    function prodApprovalState(p){ if(!p.require_confirmation) return 'none'; return p.supplier_confirmed?'approved':'pending'; }
    function prodBatchPending(bp){ return bp.filter(function(p){ return prodApprovalState(p)==='pending'; }); }
    // Productions menu badge = "unapproved of total" across every batched PO (all batches), e.g. 2 of 6.
    function setProdnBadge(){ var all=((_ppData&&_ppData.pos)||[]).filter(function(p){ return (p.batch_id==null?'':String(p.batch_id)).trim(); });
      var pend=prodBatchPending(all).length; var b=document.getElementById('pp-prodn-badge'); if(b) b.innerHTML=pend?'<span class="ex-badge">'+pend+' of '+all.length+'</span>':''; }
    function prodApprovalHtml(bp){ var pend=prodBatchPending(bp);
      var none=bp.filter(function(p){ return prodApprovalState(p)==='none'; }).length;   // POs Dock & Bay never asked to confirm: counted, not carded
      var rows=bp.filter(function(p){ return prodApprovalState(p)!=='none'; }).sort(function(a,b){ return a.po<b.po?1:-1; }).map(function(p){ var st=prodApprovalState(p);
        var pill=st==='approved'?'<span style="color:var(--pos);font-weight:600;white-space:nowrap">✓ <span>Approved</span> '+esc(p.supplier_confirmed)+(p.supplier_confirmed_by?' · '+esc(p.supplier_confirmed_by):'')+'</span>'
          :st==='pending'?'<span style="color:var(--amber);font-weight:600;white-space:nowrap">⏳ <span>Awaiting your approval</span></span>'
          :'<span class="mut" style="white-space:nowrap">No approval needed</span>';
        return '<div style="display:flex;align-items:center;gap:10px;padding:7px 11px;border:1px solid var(--line);border-radius:var(--r-sm,8px);background:#fff;font-size:12px'+(st==='pending'?';border-color:#fcd34d;background:var(--amber-bg)':'')+'">'
          +'<b style="white-space:nowrap">'+esc(p.po)+'</b>'+pill
          +(st==='pending'?'<button class="save-btn pv-prod-approve" data-po="'+esc(p.po)+'" style="margin-left:auto;background:var(--pos);border-color:var(--pos)">Approve</button>':'')+'</div>'; }).join('');
      return '<div class="bar" style="gap:10px;align-items:center;flex-wrap:wrap;margin:8px 0 6px"><b style="font-size:12px">Approvals</b>'
        +'<span class="mut tiny">'+(pend.length?pend.length+' of '+bp.length+' <span>awaiting your approval</span>':(rows?'<span>All orders in this batch are approved</span>':'<span>No orders in this batch need your approval</span>'))+(none&&rows?' · '+none+' <span>need no approval</span>':'')+'</span>'
        +(pend.length>1?'<button class="save-btn pv-prod-approve" data-all="1" style="margin-left:auto;background:var(--pos);border-color:var(--pos)">Approve all ('+pend.length+')</button>':'')+'</div>'
        +'<div style="display:flex;flex-wrap:wrap;gap:8px;margin:0 0 12px">'+rows+'</div>'; }
    // Approve one PO (or every pending PO in the batch) — same EP.submit {po_confirmed:true} the PO card posts, run sequentially, then re-render.
    ...
    function ppProductions(){
      ...
      var sel='<div class="bar" style="gap:8px;align-items:center;flex-wrap:wrap"><span class="pill-lbl" style="width:auto">Batch</span>'
        +'<select class="fci pv-prod-batch" style="width:auto;min-width:0;max-width:220px;text-align:left"><option value="">— choose —</option>'
        +batches.map(function(b){ var n=prodBatchPending(prodPOsInBatch(b)).length; return '<option value="'+esc(b)+'"'+(PORTAL_PROD_BATCH===b?' selected':'')+'>'+esc(b)+(n?' · '+n+' to approve':'')+'</option>';}).join('')+'</select>'
      ...
      return sel+prodApprovalHtml(bp)+'<div class="mut tiny" style="margin:2px 0 8px">'+bp.length+' PO'+(bp.length>1?'s':'')+' · '+d.skus.length+' SKU'+(d.skus.length>1?'s':'')+' · '+units(grand)+' units in batch '+esc(PORTAL_PROD_BATCH)+'</div>'
    ...
    function prodBatchBarcodeDl(kind,btn){ if(!PORTAL_PROD_BATCH)return; if(BC.placeholder){BC.note();return;} var orig=btn.textContent; btn.disabled=true; btn.textContent='Preparing…';
      fetch(EP.labelData+'?batch='+encodeURIComponent(PORTAL_PROD_BATCH)+'&supplier='+encodeURIComponent(STATE.supplierName)).then(function(r){return r.json();}).then(function(rows){ btn.disabled=false; btn.textContent=orig;
        ...
        BC.sheets(rows,[kind],'batch_'+PORTAL_PROD_BATCH+'_'+kind+'_barcodes.zip',btn); }).catch(function(){ppNotice('Could not load barcodes');btn.disabled=false;btn.textContent=orig;}); }
    function downloadProductionPlan(){
      var bp=prodBatchPOs(); if(!bp.length){ ppNotice('No POs in that batch.'); return; }
      ...
      setTimeout(function(){URL.revokeObjectURL(url);a.remove();},150); }
            ...
            function cardHtml(s){
              ...
              if(s.is_fob){
                ...
                var timeline='<div style="margin-top:10px;border-top:1px solid var(--line2);padding-top:8px">'
                  ...
                  +'<div style="display:flex;gap:6px;align-items:flex-start;margin-top:6px"><textarea class="fci sp-fob-note-body" spellcheck="true" autocorrect="on" autocapitalize="sentences" lang="en-GB" data-po="'+esc(po)+'" rows="2" placeholder="Add a note to the timeline… (multiple lines OK)" style="flex:1;max-width:560px;min-height:44px;text-align:left;resize:vertical;line-height:1.4"></textarea><button class="save-btn sp-fob-note-post" data-po="'+esc(po)+'">Post</button></div></div>';
          ...
          function ppSampleCard(s){
            ...
            return '<div class="samp-card" data-id="'+s.id+'" data-ref="'+esc(s.ref)+'" style="border:1px solid var(--line);border-radius:10px;padding:14px;margin-bottom:12px;background:#fff;text-align:left">'
              ...
              +'<div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-end;margin-top:12px;border-top:1px solid var(--hover);padding-top:10px">'
                ...
                +'<div>'+lbl('Tracking code')+'<input class="fci txt samp-trk" value="'+esc(s.tracking_code||'')+'" style="width:170px" placeholder="tracking…">'+(carrierTrackUrl(s.carrier,s.tracking_code)?'<div class="tiny" style="margin-top:3px"><a href="'+carrierTrackUrl(s.carrier,s.tracking_code)+'" target="_blank" rel="noopener" style="color:var(--blue);text-decoration:underline;font-weight:700" title="track with '+esc(s.carrier||'carrier')+'">track here ↗</a></div>':'')+((s.tracking_code&&/dhl/i.test(s.carrier||''))?'<div style="margin-top:4px"><span class="hz-trkpill" data-trk="'+esc(s.tracking_code)+'"></span></div>':'')+'</div>'
              ...
              +'<div style="margin-top:12px;border-top:1px solid var(--hover);padding-top:10px">'+lbl('Timeline')
                +'<div style="display:flex;gap:6px;align-items:flex-start;margin:4px 0 6px"><textarea class="fci samp-note-in" spellcheck="true" autocorrect="on" autocapitalize="sentences" lang="en-GB" rows="2" placeholder="Add a note…" style="flex:1;max-width:480px;text-align:left"></textarea><button class="save-btn samp-note-post">Post</button></div>'
          ...
          function refreshSampleCard(id){ var s=sampById(id); if(!s)return;
            ...
            setSampBadge(); }
          function ppBadgeHtml(n){ return n?'<span class="ex-badge">'+n+'</span>':''; }   // standard badge class (theme-styled; the section-row counter reads it) v27.507
          ...
          function wireSampleCard(scope,id){
            ...
            function saveLines(lines,cb){ postJSON(EP.sampleContentsBase+id+'/lines',{lines:lines},function(j){ if(j&&j.error){ppNotice(j.error);return;} loadContents(cb); }); }
            function saveDev(devs,cb){ postJSON(EP.sampleContentsBase+id+'/dev-samples',{dev_samples:devs},function(j){ if(j&&j.error){ppNotice(j.error);return;} loadContents(cb); }); }
            ...
            var save2=scope.querySelector('.samp-save-2'); if(save2)save2.onclick=function(){ var b={id:id}, F=['recipient_company','first_name','last_name','address_line1','address_line2','city','region','postcode','country','phone','tracking_code','carrier'];
              ...
              postJSON(EP.sampleUpdate,b,function(j){ if(j&&j.error){ save2.disabled=false; save2.textContent=o; ppNotice(j.error); return; } var s=sampById(id); if(s){ F.forEach(function(f){ s[f+'_2']=b[f+'_2']||''; }); } save2.textContent='✓ Saved'; setTimeout(function(){ refreshSampleCard(id); },700); }); };
          ...
          function drawProdGrid(){ var host=document.getElementById('pp-prod-grid'); if(!host)return;
            ...
            host.innerHTML='<div class="tw"><table class="ppp-tbl"><thead><tr>'+['Product','Category','Season','Sizes','Status'].map(function(h){return '<th>'+h+'</th>';}).join('')+'</tr></thead><tbody>'
              +f.map(function(p,i){ var badge=p.unread_dnb?' <span style="background:var(--neg);color:#fff;border-radius:8px;font-size:10.5px;font-weight:700;padding:0 5px">'+p.unread_dnb+'</span>':'';
                if(Number(p.dev_unaccepted)>0)badge+=' <span class="pp-acc-chip" style="background:var(--amber-bg);color:var(--amber);border-radius:8px;font-size:10px;font-weight:800;padding:1px 6px;white-space:nowrap">⏳ Accept</span>';   // v27.761: open action
          ...
          function wireProducts(){ var body=document.getElementById('pp-body');
            ...
            drawProdGrid(); }
          // v27.761 (Ben): development-request acceptance bar — open action until the supplier accepts; then a confirmation line.
          function ppProdAcceptBar(ref, it){ var pend=Number(it.dev_unaccepted)>0;
            if(pend) return '<div class="pp-acc-bar" style="margin:0 0 10px;padding:9px 12px;border-radius:var(--r-sm,8px);background:var(--amber-bg);border:1px solid #fcd34d;font-size:12px;display:flex;gap:10px;align-items:center;flex-wrap:wrap">'
          ...
          function ppProdMaster(box, ref, dispRef){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).then(function(d){ if(!d||!d.item){ box.innerHTML='<div class="mut" style="text-align:left">Not found.</div>'; return; } var it=d.item;   // v27.848 (Ben): real portal → the supplier-scoped /api/portal/product-item; admin preview falls back to the admin endpoint (still filtered client-side)
              ...
              // Component set = the product's configured components (same as admin), NOT a hardcoded 5. Falls back to the legacy 5 only if the server sent none.
              // v27.871 (Ben): mark which components THIS supplier is assigned to sample (via req_suppliers), so the size×component table can group "yours" separately from other suppliers'.
              function _compMine(c){ if(!_mySup)return true; return (c.req_suppliers||[]).some(function(s){ return String(s).trim().toLowerCase()===_mySup; }); }
              var _COMPS=(d.components!=null) ? (d.components||[]).map(function(c){ return {dim:c.dimension||('comp:'+c.id), name:c.name, mine:_compMine(c), sups:(c.req_suppliers||[])}; })
                : [['product','Product'],['packaging','Packaging'],['labels','Labels/wraps'],['polybag','Polybags'],['other','Other components']].map(function(x){ return {dim:x[0], name:x[1], mine:true, sups:[]}; });
              var _mineC=_COMPS.filter(function(c){ return c.mine; }), _otherC=_COMPS.filter(function(c){ return !c.mine; });
              ...
              function pSampleCell(s, dim){ var rel=_samplesScoped.filter(function(sm){ return (sm.sampled_aspects||[]).indexOf(dim)>=0 && (sm.sample_sizes||[]).indexOf(s.size_label)>=0; });   // v27.845: only this supplier's samples
              ...
              var sizes=(d.sizes||[]).map(function(s){ function sd(dm){ return ((s.dimensions||[]).filter(function(x){return x.dimension===dm;})[0])||{}; } var pk=sd('packaging'), ok=s.approval_status==='approved';
                var head='<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:7px"><span style="font-weight:700;font-size:15px">'+esc(s.size_label)+'</span><span style="font-size:12px;padding:1px 8px;border-radius:10px;background:'+(ok?'#ecfdf5':s.approval_status==='rejected'?'#fef2f2':'#fffbeb')+';color:'+(ok?'#065f46':s.approval_status==='rejected'?'#991b1b':'#92400e')+';font-weight:700">'+apprGlyph(s.approval_status)+' '+esc(s.approval_status||'pending')+'</span>'+(s.mapped_sku?'<span style="font-size:12px;color:var(--muted)">SKU <b style="font-family:ui-monospace,Menlo,monospace">'+esc(s.mapped_sku)+'</b></span>':'')+(ok&&s.barcode?'<span style="font-size:12px;color:var(--muted)" title="working barcode (EAN-13) — confirmed">▦ <b style="font-family:ui-monospace,Menlo,monospace">'+esc(s.barcode)+'</b></span>':'')+(ok&&s.working_sku?'<span style="font-size:12px;color:var(--muted)" title="working SKU — confirmed">working SKU <b style="font-family:ui-monospace,Menlo,monospace">'+esc(s.working_sku)+'</b></span>':'')+'</div>';   /* v27.845 (Ben): show the working barcode + working SKU per size once confirmed (approved) */
                function _compRow(cc){ var _dim=cc.dim, _sd=sd(_dim), _lbl=cc.name; if(_dim==='packaging'&&_sd.packaging_type)_lbl+=' ('+esc(_sd.packaging_type)+')'; return pDimRow(_lbl, _sd, s, _dim); }
                function _otherRow(cc){ var supTxt=(cc.sups&&cc.sups.length)?('🧵 sampled by '+cc.sups.join(', ')):'— not yet assigned to a supplier'; return '<tr style="border-top:1px solid #f4f4f5;color:var(--faint)"><td style="padding:6px 10px 6px 0;text-align:left">'+esc(cc.name)+'</td><td colspan="3" style="padding:6px 0;text-align:left" class="mut tiny">'+esc(supTxt)+'</td></tr>'; }
                function _grpHdr(txt,note){ return '<tr><td colspan="4" style="padding:9px 0 3px;font-size:9.5px;text-transform:uppercase;letter-spacing:.05em;color:var(--faint);font-weight:800;border-top:2px solid var(--line2)">'+esc(txt)+(note?' <span style="text-transform:none;font-weight:400">'+esc(note)+'</span>':'')+'</td></tr>'; }
                // v27.871 (Ben): group "yours" vs other suppliers' components when in a supplier context; admin view (no supplier) shows the flat list.
                var _rows=(_mySup && _otherC.length)
                  ? (_grpHdr('You sample','· '+(STATE.supplierName||''))+_mineC.map(_compRow).join('')+_grpHdr('Other components','· handled by other suppliers')+_otherC.map(_otherRow).join(''))
                  : _COMPS.map(_compRow).join('');
                var body='<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="color:var(--faint);font-size:10.5px;text-transform:uppercase;letter-spacing:.04em"><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Component</th><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Status</th><th style="text-align:left;padding:0 10px 4px 0;font-weight:600">Sample versions</th><th style="text-align:left;padding:0 0 4px;font-weight:600">Files</th></tr></thead><tbody>'
                  +_rows+'</tbody></table>';   /* v27.845/871 (Ben): components driven by the product's real components; grouped by supplier ownership on the portal */
              ...
              // v27.871 (Ben): a one-line summary making it obvious which components THIS supplier must sample vs which are for other suppliers.
              var _compNote='';
              if(_mySup){ var _mn=_mineC.map(function(c){return c.name;}), _on=_otherC.map(function(c){return c.name;});
                _compNote='<div style="font-size:11.5px;margin:0 0 8px;padding:7px 10px;background:var(--pos-bg,#ecfdf5);border:1px solid var(--pos-bd,#a7f3d0);border-radius:7px;text-align:left">'
                  +(_mn.length?'<b style="color:#065f46">You ('+esc(STATE.supplierName||'')+') sample: </b>'+esc(_mn.join(', ')):'<b style="color:#92400e">No components are assigned to you on this request yet.</b>')
                  +(_on.length?'<div class="mut tiny" style="margin-top:3px">Other components (handled by other suppliers): '+esc(_on.join(', '))+'</div>':'')+'</div>'; }
              box.innerHTML='<div style="max-width:660px;text-align:left">'+(sw?'<div style="margin-bottom:8px">'+sw+'</div>':'')
                ...
                +'<div style="padding:10px 0 4px"><div style="color:var(--ink-soft);font-weight:700;font-size:12.5px;margin-bottom:7px">Size variants &amp; components</div>'+_compNote+sizes+'</div></div>';
          ...
          function dlSampleLabel(opts){ try{ opts=opts||{}; var ref=String(opts.ref||''), S=2, W=500, H=320;
            ...
            c.toBlob(function(b){ if(!b){ppNotice('Could not create label');return;} b.arrayBuffer().then(function(ab){
          ...
          function ppProdSamples(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            ...
            Promise.all([getJSON(EP.productSamplesBase+encodeURIComponent(ref)), fetch((EP.productItemBase||'/api/product/item/')+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};}), fetch('/api/portal/product-components/'+encodeURIComponent(ref)).then(function(r){return r.json();}).catch(function(){return {};})]).then(function(_res){ var list=Array.isArray(_res[0])?_res[0]:[];
              ...
              function ps2ShipMenu(q){ q=(q||'').toLowerCase().trim();
                ...
                return html; }
              function ps2ShipLabel(v){ if(!v)return '— Assign later —'; if(v==='__new__')return '+ New shipment (create now)'; if(v==='not_shipped')return 'Not shipped'; var sr=SRS.filter(function(x){return String(x.id)===String(v);})[0]; return sr?(sr.ref+(srSub(sr)?' · '+srSub(sr):'')):''; }
              ...
              box.innerHTML='<div style="text-align:left"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="font-weight:700;font-size:12.5px">Sample versions</div><button class="save-btn pp-add-sample" style="background:var(--pos);color:#fff;border-color:var(--pos);font-size:12px">+ Add sample</button></div>'
                ...
                +'<div style="margin-bottom:8px"><label style="font-size:12px;display:inline-flex;align-items:center;gap:6px;max-width:100%">Sample shipment <span class="pp-ship-wrap" style="position:relative;display:inline-block;width:340px;max-width:100%"><input class="ps2-ship-inp fci" data-val="" value="— Assign later —" placeholder="search shipment / recipient / tracking…" autocomplete="off" style="width:100%;box-sizing:border-box;font-size:12px;text-align:left"><div class="ps2-ship-menu" style="display:none;position:absolute;top:100%;left:0;z-index:60;background:#fff;border:1px solid var(--line);border-radius:6px;max-height:260px;overflow:auto;width:100%;box-shadow:0 8px 22px rgba(0,0,0,.16)"></div></span></label> <span class="mut tiny">optional now — can set later per sample</span></div>'
                ...
                +'<div style="margin-top:9px"><button class="save-btn ps2-save" data-ref="'+esc(ref)+'">Submit sample version</button> <span class="ps2-msg" style="font-size:12px;margin-left:6px"></span></div></div>'+rows+'</div>';
              // (v27.553: the per-sample label is now the shared sample-card PDF, served by /api/portal/product-sample/:id/card.pdf — plain link, cookie auth)
              box.querySelectorAll('.pp-samp-status').forEach(function(sel){ sel.onchange=function(){ var id=sel.dataset.id, v=sel.value; postJSON(EP.productSample+'/'+id+'/status',{supplier_status:v},function(j){ if(j&&j.error){ppNotice(j.error);return;} var s=list.filter(function(x){return String(x.id)===String(id);})[0]; if(s)s.supplier_status=v; paint(); }); }; });   // silent
              box.querySelectorAll('.pp-ship-inp').forEach(function(inp){ var menu=box.querySelector('.pp-ship-menu[data-id="'+inp.dataset.id+'"]'); if(!menu)return;
                var s=list.filter(function(x){return String(x.id)===String(inp.dataset.id);})[0];
                function openMenu(){ menu.innerHTML=shipMenu(s,inp.value); menu.style.display=''; }
              ...
              box.querySelectorAll('.pp-meta-save').forEach(function(btn){ btn.onclick=function(){ var id=btn.dataset.id, pnl=box.querySelector('.pp-samp-meta[data-id="'+id+'"]'); if(!pnl)return;
                ...
                var aspects=Array.prototype.slice.call(pnl.querySelectorAll('.pp-meta-aspect:checked')).map(function(c){return c.value;});
                postJSON(EP.productSample+'/'+id+'/meta',{sample_sizes:sizes,sampled_aspects:aspects},function(j){ if(j&&j.error){ppNotice(j.error);return;} var s=list.filter(function(x){return String(x.id)===String(id);})[0]; if(s){ s.sample_sizes=sizes; s.sampled_aspects=aspects; } paint(); }); }; });   // silent
            ...
            getJSON(EP.productOpenSamples+(supQ?'?'+supQ.slice(1):'')).then(function(samples){
              ...
              function render(term){ term=(term||'').trim(); var lo=term.toLowerCase();
                ...
                getJSON(EP.productSkus+"?q="+encodeURIComponent(term)+supQ).then(function(rows){ rows=(Array.isArray(rows)?rows:[]).filter(function(k){return !haveSku[k.sku];});
                  ...
                  boxk.innerHTML=rows.length?rows.map(skuRow).join(''):(term?'':'<div class="mut tiny" style="padding:4px">type to search your SKUs…</div>'); }); }
              q.oninput=function(){ var v=this.value; if(kt)clearTimeout(kt); kt=setTimeout(function(){ render(v); },250); };
              render('');
              host.querySelector('.ssh-pk-cancel').onclick=function(){ host.style.display='none'; host.innerHTML=''; };
              host.querySelector('.ssh-pk-add').onclick=function(){ var msg=host.querySelector('.ssh-pk-msg'); var devs=[], skus=[];
                host.querySelectorAll('.ssh-pk-cb:checked').forEach(function(c){ var row=c.closest('.ssh-pk-row'); var qi=row&&row.querySelector('.ssh-pk-qty'); var qty=(qi&&Number(qi.value))||1; var t=c.dataset.type;
                  if(t==='dev') devs.push({id:c.value,qty:qty,ref:c.dataset.ref,colour_name:c.dataset.colour});
                  else if(t==='sku') skus.push({sku:c.value,qty:qty,description:c.dataset.desc||''});
          ...
          function ppProdTimeline(box, ref){ box.innerHTML='<div class="count" style="text-align:left">Loading…</div>';
            fetch(EP.productNotesBase+encodeURIComponent(ref)).then(function(r){return r.json();}).then(function(notes){ shortNotes(notes); notes=Array.isArray(notes)?notes:[];
              ...
              var pb=box.querySelector('.pp-prod-post'); if(pb)pb.onclick=function(){ var v=box.querySelector('.pp-prod-note').value.trim(); if(!v)return; postJSON(EP.productNote,{ref:ref,body:v},function(j){ if(j&&j.error){ppNotice(j.error);return;} ppProdTimeline(box,ref); }); };
            }).catch(function(e){ box.innerHTML='<div style="color:var(--neg);text-align:left">Failed: '+esc(e&&e.message||e)+'</div>'; }); }
          ...
          function unreadMarkRead(type,ref,noteId,cb){ var url,body;
            ...
            postJSON(url,body,function(){ var D=_ppData||{};   // keep the ✉ badge honest without a full bootstrap refetch
              ...
              try{ setSampBadge();setPosBadge();setShipBadge();setProdBadge();setProdnBadge(); }catch(e){}
              if(cb)cb(); }); }
          ...
          function ppQuality(){
            ...
            // Prod # / batch # / PO options come from THIS supplier's PO history (_ppData.pos). Dropdown-only
            // (no free text — batch numbers especially must be chosen, not typed); prod + batch sort largest→smallest.
            ...
            return '<div style="max-width:920px;text-align:left">'
              ...
              +'<div style="'+stepCss+'"><div style="font-weight:600;margin-bottom:8px"><span style="'+numCss+'">2</span>Specify the file type</div>'
                +'<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:10px"><label class="tiny mut" style="min-width:62px">File type</label>'
                  +'<select class="fci" id="pq-type" style="text-align:left"><option value="">— select file type —</option>'+TYPES.map(function(t){return '<option value="'+esc(t)+'">'+esc(t)+'</option>';}).join('')+'</select></div>'
                +'<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><label class="tiny mut" style="min-width:62px">Assign to</label>'
                  +sel('pq-prod','— production —',prods,'150px')+sel('pq-batch','— batch —',batches,'150px')+sel('pq-po','— PO (optional) —',poList,'190px')+'</div>'
          ...
          function wireQuality(){
            var fileEl=document.getElementById('pq-file'), drop=document.getElementById('pq-drop'), fname=document.getElementById('pq-fname');
            var typeEl=document.getElementById('pq-type'), prodEl=document.getElementById('pq-prod'), batchEl=document.getElementById('pq-batch'), poEl=document.getElementById('pq-po');
            var up=document.getElementById('pq-up'), msg=document.getElementById('pq-msg'), hint=document.getElementById('pq-hint');
            var _file=null;
            function refreshBtn(){ var ok=!!_file && !!(typeEl&&typeEl.value); if(up){ up.disabled=!ok; up.style.background=ok?'#1a1a1a':'#cbd5e1'; up.style.cursor=ok?'pointer':'not-allowed'; } if(hint)hint.style.display=ok?'none':''; }
            function setFile(f){ _file=f||null; if(fname)fname.textContent=_file?('✓ '+_file.name+' ('+Math.max(1,Math.round(_file.size/1024))+' KB)'):''; refreshBtn(); }
            // Step 1 — drag & drop + click to browse
            if(drop){
              drop.onclick=function(){ if(fileEl)fileEl.click(); };
              drop.ondragover=function(e){ e.preventDefault(); drop.style.borderColor='#1a1a1a'; drop.style.background='#f8fafc'; };
              drop.ondragleave=function(){ drop.style.borderColor='#cbd5e1'; drop.style.background=''; };
              drop.ondrop=function(e){ e.preventDefault(); drop.style.borderColor='#cbd5e1'; drop.style.background=''; var f=((e.dataTransfer&&e.dataTransfer.files)||[])[0]; if(f)setFile(f); };
            }
            if(fileEl)fileEl.onchange=function(){ setFile((fileEl.files||[])[0]||null); };
            // Step 2 — type change enables the button; picking prod/batch clears the required-flash
            if(typeEl)typeEl.onchange=refreshBtn;
            function clearFlash(el){ if(el){ el.style.borderColor=''; el.style.boxShadow=''; } }
            if(prodEl)prodEl.onchange=function(){ clearFlash(prodEl); clearFlash(batchEl); };
            if(batchEl)batchEl.onchange=function(){ clearFlash(prodEl); clearFlash(batchEl); };
            ...
            if(up)up.onclick=function(){
              if(!_file){ if(msg)msg.textContent='Attach a file first.'; return; }
              if(!typeEl.value){ if(msg)msg.textContent='Choose a file type.'; return; }
          ...
          function wireSpecs(){ document.querySelectorAll('.sp-confirm').forEach(function(b){ b.onclick=function(){ var id=b.dataset.id; b.disabled=true; b.textContent='…';
            fetch(EP.specApprove,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({spec_id:id})}).then(function(r){return r.json();}).then(function(j){ if(j&&j.error){ b.disabled=false; b.textContent='Confirm'; ppNotice(j.error); return; }
          ...
          function renderPP(){ if(!_ppData)return; var body=document.getElementById('pp-body');
            ...
            setSampBadge(); setPosBadge(); setShipBadge(); setProdBadge(); setSpecBadge(); try{ setProdnBadge(); }catch(e){} try{ renderPortalNotif(); }catch(e){} ppSyncSec(); if(PP_LANG==='zh')try{ ppTranslate(document.body); }catch(e){}   // re-sync after the badges are set (section counts, v27.506)
            ...
            if(PORTAL_TAB==='productions'){ body.innerHTML=ppProductions();
              ...
              body.querySelectorAll('.pv-prod-approve').forEach(function(btn){ btn.onclick=function(){ prodApprove(btn.dataset.all?prodBatchPending(prodBatchPOs()).map(function(p){return p.po;}):[btn.dataset.po], btn); }; });
            ...
            if(PORTAL_TAB==='barcodes'){
              ...
              function bcBatchDl(kinds,name,btn){ if(!PORTAL_BC_BATCH)return; if(BC.placeholder){BC.note();return;} btn.disabled=true;
                fetch(EP.labelData+'?batch='+encodeURIComponent(PORTAL_BC_BATCH)+'&supplier='+encodeURIComponent(STATE.supplierName)).then(function(r){return r.json();}).then(function(rows){ btn.disabled=false;
                  if(rows&&rows.error){ppNotice(rows.error);return;} if(!rows||!rows.length){ppNotice('No barcodes found for batch '+PORTAL_BC_BATCH);return;}
                  BC.sheets(rows,kinds,'batch_'+PORTAL_BC_BATCH+'_'+name+'_barcodes.zip',btn); }).catch(function(){ppNotice('Could not load barcodes');btn.disabled=false;}); }
...
scope.querySelectorAll('.pp-dl-cd').forEach(function(btn){ btn.onclick=function(){ if(BC.placeholder){BC.note();return;} btn.disabled=true; fetch(EP.labelData+'?skus='+encodeURIComponent(btn.dataset.skus)).then(function(r){return r.json();}).then(function(rows){ btn.disabled=false; if(!rows||!rows.length||rows.error){ppNotice('No crossdock barcodes found');return;} BC.crossdock(rows,btn.dataset.po,btn.dataset.do,btn.dataset.client,btn.dataset.address,btn,btn.dataset.po+'_crossdock_labels.zip'); }).catch(function(){ppNotice('Could not load crossdock labels');btn.disabled=false;}); }; });
              ...
              scope.querySelectorAll('.pp-note-post').forEach(function(btn){ var _ta=pick('pp-note-body',btn.dataset.po); var att=_ta?hzTlAttach(_ta,{after:_ta.parentNode,upload:hzTlUploader('po',btn.dataset.po),maxWidth:'480px'}):null;   // v27.571: photos / documents on the message
                ...
                hzTlSend(att,v,function(bodyTxt,attId,next){ postJSON(EP.note,{po:po,supplier_id:sid,body:bodyTxt,author_kind:'supplier',author_email:by,attachment_id:attId||null},next); },function(){
              ...
              scope.querySelectorAll('.pp-cd-grid').forEach(function(inp){ var t;
                ...
                inp.onchange=function(){ var v=inp.value; if(!/^\d{4}-\d{2}-\d{2}$/.test(v))return;
                  ...
                  t=setTimeout(function(){ postJSON(EP.submit,{po:po,supplier_id:sid,submitted_by:by,completion_date:v},function(){
                    ...
                    setManageBadge(po); setPosBadge(); setShipBadge(); try{ setProdnBadge(); }catch(e){}
                  }); },900); }; });
              ...
              scope.querySelectorAll('.pp-ac-add').forEach(function(btn){ btn.onclick=function(){ var box=btn.closest('.ppx'), po=btn.dataset.po, row=btn.closest('tr[id^="pp-"]');
                ...
                if(!desc.trim()&&!q&&!pr){ ppNotice('Enter a description, quantity or price for the additional cost.'); return; } btn.disabled=true;
