...
console.log('render + stk2 cells:', E(`(function(){ try{ if(typeof renderMain==='function')renderMain(); }catch(e){ return 'render-err:'+e.message; } var cells=document.querySelectorAll('td.stk2'); var out=[]; for(var i=0;i<Math.min(4,cells.length);i++){ out.push(cells[i].textContent.replace(/\\s+/g,' ').trim()); } return 'nCells='+cells.length+' :: '+out.join('  ||  '); })()`));
console.log('header stk2:', E(`(function(){ var th=document.querySelector('th.stk2'); return th?th.textContent:'none'; })()`));
