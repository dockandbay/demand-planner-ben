-- 145b: back-fill the Payments Report so every existing payment run that has NO bank amount / currency yet is
-- treated as paid in USD at its base total. Bank amount = the run's base (USD) total, bank currency = 'USD'.
-- Idempotent + non-destructive: runs already carrying a bank amount or currency are left untouched (COALESCE keeps
-- existing values; only blanks are filled). Safe to run on both sandbox and live.
--
-- A "run" = one bank payment = (payment date, supplier). The total mirrors the derived Payments Report exactly:
-- PO completion + balance milestones (paid dates set) + the deposit register + Other payments, for suppliers whose
-- master kind = 'supplier'. Start deposits are excluded (they are a draw against a register deposit, not cash).
WITH lines AS (
...
SELECT run_date, supplier, total, 'USD' FROM runs
ON CONFLICT (run_date, supplier) DO UPDATE SET
  paid_amount   = COALESCE(planner.payment_fx.paid_amount, excluded.paid_amount),
  paid_currency = CASE WHEN COALESCE(planner.payment_fx.paid_currency,'')='' THEN excluded.paid_currency ELSE planner.payment_fx.paid_currency END,
  updated_at    = now();
