...
- Generates a multi-page PDF, ONE label per carton: full cartons (MASTER, qty=carton_qty) + one ODD/MIXED carton for the remainder (overage). Layout matches the template: The Paper Store / PO# <client_po_ref> - <DC#> DISTRIBUTION CENTER / DC address / CTN# n OF total / ITEM# SKU / DESC / UPC (EAN-13 barcode) / MASTER QTY PCS / N.W (blank) / G.W (us_carton_weight, lb) / MEAS (US carton dims, in) / INNER QTY (blank). "MASTER CARTON" / "ODD / MIXED CARTON" printed in red.
- Server: GET /api/supply/paperstore-labels/:po assembles carton records (floor(qty/carton_qty) full + remainder odd, sequential CTN#, US carton weight/dims; odd-carton G.W pro-rated). PO-1845589 → 5 labels (4 master + 1 odd). No migration/dep.
...
- (Point 1 — the "download all" ZIP showing the same code in both supplier folders — was already fixed in v26.597: each folder now renders that supplier's code on the label + -CODE filename. This is live once Diviyaj deploys ≥v26.597.)
...
- New Disc (EU) column from SKUM.disc.eu; the cover sum EXCLUDES months after the EU discontinue date, so no stock is sent to cover post-discontinue demand (discontinued SKUs → suggested 0).
...
## v26.581.0 — US Geneva 3PL: full accounting allocation (PDF + fee-type map)
- Automatic PDF invoice parse (_tplGenevaPdfItems via pdf-parse) → 22 fee-type totals, reconciles to the invoice total. Fee-code→Xero-account map (GENEVA_ACCT, from Ben's …GENEVA.csv): D2C+personalization→Shopify, FBA→Amazon, B2B(+LTL)→split Wholesale/Faire by B2B UNIT count, embroidering→Co-Brand, SSCC/embroidering-whsl→Wholesale, admin/packaging/returns/inbound/labour→Other fees, storage→Storage fees. FREIGHT (SHIPPING) allocated per-order to the order's Cin7 CostCentre. New tpl/map branch for tpl=us_geneva returns the per-account breakdown (same shape as the generic map → existing Review + Xero CSV reuse). Needs both the freight xlsx + the invoice PDF uploaded for the period.
- Verified vs June invoice #INV300658007: allocation reconciles to $116,106.52; fulfilment per account matches Ben's sheet to the cent (Shopify 16,556.28, Co-Brand 7,375.28, Amazon 804, Other fees 8,776.32, Storage 8,172.00). Freight-per-CC + B2B split need the live Cin7 import (untestable offline).
- NEW DEPENDENCY: pdf-parse (pure-JS, no native bindings) — Diviyaj: npm install on deploy.

## v26.580.0 — 3PL Invoicing: US Geneva (G10) parser
- US Geneva uses a different layout to iFulfilment/Coghlans: per-order FULFILMENT fees on a "<MONTH> - Pick Detail" sheet and per-order FREIGHT (Carrier Amount Due) on a "<MONTH> - Small Parcel" sheet, with ONE workbook holding every month. New _tplGenevaRows() merges the two sheets for the invoice period by OrderID → the same {reference, freight, fulfilment} rows the existing map/sweep/Xero-CSV pipeline consumes. Wired into parse (period-filtered Analyse), map, sweep and export when tpl=us_geneva. Fulfilment = sum of every "… Fee" column; freight = per-row Carrier Amount Due (the appended pivot summary has no OrderID → skipped).
- Verified against the June invoice #INV300658007: freight $72,155.64 matches the PDF SHIPPING line to the cent; fulfilment $19,584.68 (outbound fees + personalization); per-order allocatable $91,740.32; the remaining $24,366.20 is overhead (storage/packaging/admin/embroidering/returns/labour) = TODO recap lines. 6,005 orders.
...
- 2027 YoY basis unchanged (still vs 2026 forecast) per Ben.
...
- Display-only; no logic/schema change. `artifact_v16.7.html` only.
...
- Follows the plan: switching country/channel or editing a forecast re-renders the card live. Button only shows on DEMAND ▸ Plan. Reuses `revMonthly` + fetches targets on open. `artifact_v16.7.html` only (no schema change).
...
- **Diviyaj:** run migration **164** (two tables, additive). The 5-min worker runs in-process (`setInterval`); pending queued rows are picked up after a restart, so nothing is lost.
...
- Note: totals across mixed-currency suppliers are flagged in the count line — filter to a supplier for a clean sum.
...
- "⤓ Download label" is now left-aligned, right after the colour/quality ticks (was pushed to the far right).
...
- The product detail endpoint now runs its queries in **parallel** (3 round-trips instead of ~8) — the first tab
  loads in ~1s on sandbox (was 3s+), faster still on live.
- Removed the inline "⏳ Loading master data…" text — the **version-badge spinner** already signals the load.
...
- Removed the redundant inline "showing high-priority actions — loading the rest…" banner — the version-badge
  spinner already signals that loading is in progress.
- The **ACTIONS sub-tab** now shows a **red counter** of open high-priority actions (matching the top-level ACTIONS
  tab badge).
- The version-badge loading spinner now reads **"loading"** alongside the spinner.
...
- Saving a supplier failed ("column cin7_member_id is of type bigint but expression is of type text") — the patch
  allowlist cast cin7_member_id as text, but it's a bigint column. Corrected to bigint. This had blocked toggling
  the new "Include for product development" flag (and any supplier edit submitting cin7_member_id).

## v26.025 - Add-new-product: supplier uses the master-data picker

- The New-product form's Supplier field now uses the same search-popup picker as master data / the grid cell
  (select-only mode, since there's no ref yet); the chosen supplier's code still goes into the reference.

## v26.024 - Product detail sub-tabs match PO grid + master-data supplier picker

- Product detail sub-tabs (Master data / Samples / Timeline / Documents) now use the PO grid's underline .rtab
  style. Master-data Supplier uses the same inline picker popup as the grid cell.
